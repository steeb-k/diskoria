//! Read-only full-disk surface scan (parity with `_original/Services/SurfaceTestService.cs`).
...
/// Matches WPF `SurfaceTestProgress` fields used by the UI.
#[derive(Clone, Debug)]
...
pub fn spawn_surface_test(
    ...
    std::thread::spawn(move || {
        ...
        if let Err(e) = run_physical_drive_surface_test(&device_path, total_blocks, &cancel, &tx) {
            log::warn!(target: "jf_surface", "surface worker: run returned Err, sending Error to UI");
            let _ = tx.send(SurfaceTestMsg::Error(e));
        } else {
            log::debug!(target: "jf_surface", "surface worker: run returned Ok");
        }
        log::debug!(target: "jf_surface", "surface worker: sending Completed");
