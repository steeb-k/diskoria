//! Window icon, fonts, logo textures, custom title bar, Win32 resize + DWM rounding (from winui_egui_example).
...
pub fn window_icon_from_image_bytes(data: &[u8]) -> egui::IconData {
...
/// Dark-mode logo plus RGB-inverted “light” variant (Copynaut / template workflow).
pub fn load_logo_textures(
    ...
    let logo = ctx.load_texture("jf_storage_tester_logo", color_image, egui::TextureOptions::LINEAR);
    ...
    let logo_light = ctx.load_texture("jf_storage_tester_logo_light", img_inv, egui::TextureOptions::LINEAR);
    (Some(logo), Some(logo_light), size)
}

/// Single texture from PNG bytes (e.g. repo-root `appicon.png` for About).
...
/// Hook WM_NCHITTEST for resize borders when using undecorated window (template pattern).
#[cfg(windows)]
...
#[cfg(windows)]
pub fn apply_win11_rounded_corners(cc: &eframe::CreationContext<'_>) {
    ...
    const WINDOW_CORNER_ROUND: u32 = 2;
...
#[cfg(not(windows))]
pub fn apply_win11_rounded_corners(_cc: &eframe::CreationContext<'_>) {}
...
pub fn draw_titlebar(ctx: &egui::Context, t: &Theme) {
    ...
    egui::Area::new(Id::new("jf_tb_drag"))
        .fixed_pos(drag_rect.min)
        .order(egui::Order::Foreground)
        .show(ctx, |ui| {
            let drag_r = ui.interact(drag_rect, Id::new("jf_tb_drag_i"), Sense::click_and_drag());
            if drag_r.dragged() {
    ...
    egui::Area::new(Id::new("jf_tb_controls"))
        .fixed_pos(min_rect.left_top())
        .order(egui::Order::Foreground)
        .show(ctx, |ui| {
            let min_r = ui.interact(min_rect, Id::new("jf_tb_min"), INTERACT_MANUAL_FOCUS);
            let max_r = ui.interact(max_rect_area, Id::new("jf_tb_max"), INTERACT_MANUAL_FOCUS);
            let close_r = ui.interact(close_rect, Id::new("jf_tb_close"), INTERACT_MANUAL_FOCUS);
