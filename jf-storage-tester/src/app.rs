...
impl JfStorageTesterApp {
    pub fn new(cc: &eframe::CreationContext) -> Self {
        ...
        #[cfg(windows)]
        {
            ...
            if let Ok(window_handle) = cc.window_handle() {
                if let RawWindowHandle::Win32(win32) = window_handle.as_raw() {
                    crate::chrome::install_win32_resize(win32.hwnd.get());
                }
            }
        }

        static LOGO_PNG: &[u8] = include_bytes!("../../applogo.png");
        let (logo, logo_light, logo_size) = load_logo_textures(&cc.egui_ctx, LOGO_PNG);

        static ABOUT_PNG: &[u8] = include_bytes!("../../appicon.png");
        let about_appicon = crate::chrome::load_appicon_texture(&cc.egui_ctx, ABOUT_PNG);
        ...
        let system_dark = match cc.egui_ctx.system_theme() {
        ...
        apply_visuals(&cc.egui_ctx, dark, accent_color);
    ...
    fn draw_update_download_confirm(&mut self, ctx: &egui::Context, dark: bool) {
        ...
        match two_button_modal(
            ...
            TwoButtonModalParams {
                overlay_id: Id::new("jf_update_dl_overlay"),
                dialog_id: Id::new("jf_update_dl_dialog"),
                width: 400.0,
                height: 200.0,
                title: "Update available",
                body: &body,
                cancel_id: Id::new("jf_update_dl_cancel"),
                cancel_label: "Cancel",
                confirm_id: Id::new("jf_update_dl_ok"),
    ...
    fn draw_update_alert(&mut self, ctx: &egui::Context, dark: bool) {
        ...
        if one_button_modal(
            ...
            OneButtonModalParams {
                overlay_id: Id::new("jf_update_alert_overlay"),
                dialog_id: Id::new("jf_update_alert_dialog"),
                width: 400.0,
                height: 200.0,
                title: &title,
                body: &body,
                ok_id: Id::new("jf_update_alert_ok"),
                ok_label: "OK",
            },
    ...
    fn speed_test_temp_path(drive_letter: &str) -> String {
        ...
        let id = std::time::SystemTime::now()
            ...
            .unwrap_or(0);
        format!(
            "{}JFStorageTester_SpeedTest_{}.tmp",
            root, id
        )
    }
    ...
    fn draw_stop_test_confirm(&mut self, ctx: &egui::Context, dark: bool) {
        ...
        match two_button_modal(
            ...
            &t,
            TwoButtonModalParams {
                overlay_id: Id::new("jf_stop_test_overlay"),
                dialog_id: Id::new("jf_stop_test_dialog"),
                width: 360.0,
                height: 160.0,
                title: "Stop sector test?",
                body: "The scan will be aborted. Progress on the map reflects reads so far.",
                cancel_id: Id::new("jf_stop_test_no"),
                cancel_label: "No",
                confirm_id: Id::new("jf_stop_test_yes"),
    ...
    fn spawn_drive_enumeration(&mut self, ctx: &egui::Context) {
        log::debug!(target: "jf_surface", "spawn_drive_enumeration: starting WMI thread");
    ...
    fn poll_drive_enumeration(&mut self, ctx: &egui::Context) {
        ...
        match rx.try_recv() {
            ...
            Ok(Err(e)) => {
                log::warn!(target: "jf_surface", "poll_drive_enumeration: error {e}");
            ...
            Err(mpsc::TryRecvError::Empty) => {}
            Err(mpsc::TryRecvError::Disconnected) => {
                log::warn!(target: "jf_surface", "poll_drive_enumeration: channel disconnected");
                self.drives_loading = false;
                self.drive_poll_rx = None;
            }
        }
    ...
    fn start_surface_test(&mut self, ctx: &egui::Context) {
        if self.drives.is_empty() {
            log::debug!(target: "jf_surface", "start_surface_test: skipped (no drives)");
            return;
        }
        if self.surface_test_running {
            log::debug!(target: "jf_surface", "start_surface_test: skipped (already running)");
            return;
        }
        let sel = self.selected_drive.min(self.drives.len().saturating_sub(1));
        ...
        log::debug!(target: "jf_surface", "start_surface_test: worker spawned, running=true");
        self.sector_focus = Some(0);
        ctx.request_repaint();
    }

    fn stop_surface_test_user(&mut self) {
        log::info!(target: "jf_surface", "stop_surface_test_user: user Stop / cancel");
        if let Some(c) = &self.surface_test_cancel {
            c.store(true, Ordering::SeqCst);
        }
        self.surface_test_running = false;
    ...
    fn start_destructive_test(&mut self, ctx: &egui::Context) {
        ...
        log::debug!(target: "jf_surface", "start_destructive_test: worker spawned");
        self.destructive_focus = Some(0);
        ctx.request_repaint();
    }

    fn stop_destructive_test_user(&mut self) {
        log::info!(target: "jf_surface", "stop_destructive_test_user: user cancelled");
        ...
        for msg in batch {
            match msg {
                ...
                DestructiveTestMsg::Error(e) => {
                    log::warn!(target: "jf_surface", "poll_destructive_test: Error {e}");
    ...
    fn draw_destructive_start_confirm(&mut self, ctx: &egui::Context, dark: bool) {
        ...
        match two_button_modal(
            ...
            &t,
            TwoButtonModalParams {
                overlay_id: Id::new("jf_dest_start_overlay"),
                dialog_id: Id::new("jf_dest_start_dialog"),
                width: 420.0,
                height: 200.0,
                title: "Destroy all data on this drive?",
                body: &body,
                cancel_id: Id::new("jf_dest_start_cancel"),
                cancel_label: "Cancel",
                confirm_id: Id::new("jf_dest_start_confirm"),
                confirm: ModalConfirmPrimary::Danger {
                    label: "Continue",
                },
            },
    ...
    fn draw_destructive_stop_confirm(&mut self, ctx: &egui::Context, dark: bool) {
        ...
        match two_button_modal(
            ...
            &t,
            TwoButtonModalParams {
                overlay_id: Id::new("jf_dest_stop_overlay"),
                dialog_id: Id::new("jf_dest_stop_dialog"),
                width: 380.0,
                height: 160.0,
                title: "Stop destructive test?",
                body: "The test will be aborted. Partially-overwritten sectors will not be verified.",
                cancel_id: Id::new("jf_dest_stop_cancel"),
                cancel_label: "No",
                confirm_id: Id::new("jf_dest_stop_confirm"),
                confirm: ModalConfirmPrimary::Danger {
                    label: "Yes, Stop",
                },
            },
        ...
        for msg in batch {
            match msg {
                ...
                SurfaceTestMsg::Error(e) => {
                    log::warn!(target: "jf_surface", "poll_surface_test: Error {e}");
    ...
    fn draw_sidebar(&mut self, ctx: &egui::Context, dark: bool) {
        ...
        egui::SidePanel::left("jf_sidebar")
            .resizable(false)
    ...
    fn destructive_primary_id() -> Id {
        Id::new("jf_destructive_primary")
    }

    fn destructive_unlock_id() -> Id {
        Id::new("jf_destructive_unlock")
    }
    ...
    fn draw_destructive_page(
        ...
        ui.horizontal(|ui| {
            ...
            ui.add_space(16.0);
            ui.push_id("jf_destructive_refresh", |ui| {
        ...
        // Drive combo
        let combo_id = Id::new("jf_destructive_drive_combo");
    ...
    fn draw_destructive_result_overlay(&self, ctx: &egui::Context) {
        ...
        let screen = ctx.screen_rect();
        let painter = ctx.layer_painter(LayerId::new(Order::Foreground, Id::new("jf_destructive_overlay")));
    ...
    fn draw_sector_page(
        ...
        ui.horizontal(|ui| {
            ...
            ui.label(egui::RichText::new(subtitle).size(14.0).color(t.txt_sec));
            ui.add_space(16.0);
            ui.push_id("jf_sector_refresh", |ui| {
                let refresh = ui.add_enabled(
                    !self.any_test_running(),
        ...
        let combo_id = Id::new("jf_sector_drive_combo");
        let options: Vec<String> = self.drives.iter().map(|d| d.summary.clone()).collect();
        ...
        #[cfg(windows)]
        {
            ...
            if self.surface_test_running {
                ...
                let btn_r = ui.interact(btn_rect, Id::new("jf_sector_primary"), Sense::click());
                let bg = if btn_r.hovered() || stop_focused {
                    Color32::from_rgb(
                        CLOSE_HOVER_BG.r().saturating_add(12),
                        CLOSE_HOVER_BG.g().saturating_add(8),
                ...
                let btn_r = ui.interact(btn_rect, Id::new("jf_sector_primary"), btn_sense);
                let bg = if !can_start {
                    Color32::from_rgba_premultiplied(
                        t.accent.r() / 2,
                        t.accent.g() / 2,
    ...
    fn draw_sector_result_overlay(&self, ctx: &egui::Context, _t: &Theme) {
        ...
        let screen = ctx.screen_rect();
        let painter = ctx.layer_painter(LayerId::new(Order::Foreground, Id::new("jf_sector_overlay")));
        painter.rect_filled(screen, 0.0, Color32::from_rgba_unmultiplied(0, 0, 0, 220));
    ...
    fn draw_settings_theme(
        ...
        for (i, (label, pref)) in options.iter().enumerate() {
            ...
            let resp = ui.interact(seg, Id::new(("jf_theme_seg", i)), INTERACT_MANUAL_FOCUS);
        ...
        for (i, (label, pref)) in accent_options.iter().enumerate() {
            ...
            let resp = ui.interact(seg, Id::new(("jf_accent_seg", i)), INTERACT_MANUAL_FOCUS);
        ...
        if self.accent_source == AccentSourcePref::Palette {
            for (idx, col) in ACCENT_PALETTE.iter().enumerate() {
                ...
                let sw_resp =
                    ui.interact(sw_rect, Id::new(("jf_accent_swatch", idx)), INTERACT_MANUAL_FOCUS);
                scroll_to_focused(
                    &mut self.pending_scroll_rect,
                    sw_rect,
                    sw_focused,
                ...
                if sw_resp.hovered() {
                    ...
                    show_tooltip_text(ctx, Id::new(("jf_accent_swatch_tt", idx)), mouse_pos, t, &tt);
                }
            }
        } else {
            ui.painter().text(
    ...
    fn draw_speed_page(
        ...
        ui.horizontal(|ui| {
            ...
            ui.add_space(16.0);
            ui.push_id("jf_speed_refresh", |ui| {
                let refresh = ui.add_enabled(
                    !self.any_test_running(),
                    egui::Button::new(egui::RichText::new("⟳ Refresh").color(t.txt_pri)),
                );
        ...
        let volume_combo_id = Id::new("jf_speed_volume_combo");
        let pairs = Self::speed_volume_pairs(&self.drives);
...
fn nice_y_max(max_speed: f64) -> f64 {
    ...
    for &step in &[100.0_f64, 200.0, 300.0, 500.0, 750.0, 1000.0, 1500.0, 2000.0, 3000.0, 5000.0, 7000.0, 10000.0] {
        if step >= headroom {
            return step;
        }
    }
...
impl eframe::App for JfStorageTesterApp {
