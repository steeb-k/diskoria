...
//! Pattern aligned with copynaut `ui/focus.rs` / WINDOWS11_EGUI_STYLE_GUIDE integration recipe.

use egui::{ComboBox, Context, EventFilter, Id, Key, Modifiers, Rect, Response, Ui};
...
/// Like [`bind_text_focus_slot`] for a [`ComboBox`] button id, but does not steal focus back to the
/// button while [`ComboBox::is_open`] (otherwise arrow keys in the list stop working; see copynaut).
/// egui closes [`ComboBox`] popups when the **pointer** clicks a list row; Enter/Space use
/// [`Response::clicked`] without a pointer click, so the popup stays open unless we close it here.
#[inline]
pub fn close_popup_if_selectable_clicked(ctx: &Context, item_response: &Response) {
