//! File-based read/write benchmarks (parity with `_original/Services/SpeedTestService.cs`).

use crate::detected_drive::{BusKind, MediaKind};

/// Progress reported to the UI (WPF `SpeedTestProgress`).
#[derive(Clone, Debug)]
...
/// Map Rust media/bus to the same buckets as WPF `StorageType`.
...
mod win {
    ...
    fn run_sequential_read_pass(
        ...
        while read_b < file_size {
            ...
            if read_b % (32 * SEQ_BLOCK as u64) == 0 || read_b == file_size {
                let elapsed = sw.elapsed().as_secs_f64().max(1e-9);
