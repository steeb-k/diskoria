fn main() {
    ...
    #[cfg(windows)]
    {
        // winresource links the icon + manifest globally (`rustc-link-lib`), so
        // the app.manifest's `requireAdministrator` also lands on the `cargo test`
        // harness exe — which then fails to launch with "requires elevation"
        // (os error 740) in an unelevated/CI shell. Setting
        // DISKORIA_SKIP_RESOURCE=1 builds without the resource so the test
        // binaries run unprivileged. Normal `cargo build`/release keep it. See
        // docs/known-issues.md (KI-10).
