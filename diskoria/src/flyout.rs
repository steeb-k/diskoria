...
const CM_H: f32 = 128.0; // 6 + 36 + 36 + 8 + 36 + 6
...
fn draw_context_menu_ui(ctx: &egui::Context, t: &crate::theme::Theme) -> Option<ContextMenuAction> {
    ...
    egui::CentralPanel::default()
        ...
        .show(ctx, |ui| {
            ...
            // ── "New Window" ─────────────────────────────────────────────────
            let new_win_rect = Rect::from_min_size(
                Pos2::new(1.0, pad + item_h),
                Vec2::new(CM_W - 2.0, item_h),
            );
            let new_win_resp = ui.interact(new_win_rect, egui::Id::new("cm_new_window"), Sense::click());
            if new_win_resp.hovered() {
                ui.painter().rect_filled(new_win_rect, 3.0, t.hover);
            }
            ui.painter().text(
                Pos2::new(text_x, new_win_rect.center().y),
                Align2::LEFT_CENTER,
                "New Window",
                FontId::new(13.0, egui::FontFamily::Proportional),
                t.txt_pri,
            );
            if new_win_resp.clicked() {
                action = Some(ContextMenuAction::NewWindow);
            }

            // ── Separator ────────────────────────────────────────────────────
            let sep_y = pad + 2.0 * item_h + 3.5;
            ui.painter().hline(
                8.0..=(CM_W - 8.0),
                sep_y,
                Stroke::new(1.0, t.border),
